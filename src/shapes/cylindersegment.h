...
#include <mitsuba/render/shape.h>
    ...
    #include "optix/cylindersegment.cuh"
#endif
...
.. _shape-cylindersegment:

CylinderSegment (:monosp:`cylindersegment`)
----------------------------------------------------
 ...
 * - e0
   - |point|
   - Object-space starting point of the cylindersegment's centerline.
     (Default: (0, 0, 0))
 * - e1
   - |point|
   - Object-space endpoint of the cylindersegment's centerline (Default: (0, 0, 1))
 * - radius
   - |float|
   - Radius of the cylindersegment in object-space units (Default: 1)
 * - flip_normals
   - |bool|
   -  Is the cylindersegment inverted, i.e. should the normal vectors
...
.. subfigstart::
.. subfigure:: ../../resources/data/docs/images/render/shape_cylindersegment_onesided.jpg
   :caption: CylinderSegment with the default one-sided shading
.. subfigure:: ../../resources/data/docs/images/render/shape_cylindersegment_twosided.jpg
   :caption: CylinderSegment with two-sided shading
.. subfigend::
   :label: fig-cylindersegment

This shape plugin describes a simple cylindersegment intersection primitive.
It should always be preferred over approximations modeled using
triangles. Note that the cylindersegment does not have endcaps -- also,
...
A simple example for instantiating a cylindersegment, whose interior is visible:

.. code-block:: xml

    <shape type="cylindersegment">
...
class CylinderSegment final : public Shape<Float, Spectrum> {
public:
    ...
    CylinderSegment(const Properties &props) : Base(props) {
    ...
    CylinderSegment(ScalarPoint3f e0, ScalarPoint3f e1,
    ...
    ScalarBoundingBox3f bbox() const override {
        ...
        /* To bound the cylindersegment, it is sufficient to find the
    ...
    ScalarBoundingBox3f bbox(ScalarIndex /*index*/, const ScalarBoundingBox3f &clip) const override {
        ...
        /* Now forget about the cylindersegment ends and intersect an infinite
           cylindersegment with each bounding box face, then compute a bounding
        ...
        // Project the cylindersegment direction onto the plane
    ...
    PreliminaryIntersection3f ray_intersect_preliminary(const Ray3f &ray_,
        ...
        // CylinderSegment doesn't intersect with the segment on the ray
        Mask out_bounds = !(near_t <= maxt && far_t >= mint); // NaN-aware conditionals

        Double z_pos_near = oz + dz*near_t,
               z_pos_far  = oz + dz*far_t;

        // CylinderSegment fully contains the segment of the ray
    ...
    void optix_prepare_geometry() override {
        if constexpr (is_cuda_array_v<Float>) {
            if (!m_optix_data_ptr)
                m_optix_data_ptr = cuda_malloc(sizeof(OptixCylinderSegmentData));

            OptixCylinderSegmentData data = { bbox(), m_to_world, m_to_object,
                                       m_length, m_radius, m_flip_normals };

            cuda_memcpy_to_device(m_optix_data_ptr, &data, sizeof(OptixCylinderSegmentData));
        }
    }
#endif
...
MTS_IMPLEMENT_CLASS_VARIANT(CylinderSegment, Shape)
